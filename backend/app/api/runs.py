...
def _build_prompt(question: str, conversation: list[dict], documents: list[dict]) -> str:
    parts = []
    if documents:
        docs_text = "\n\n".join(f"[Document {d.get('id', i)}]\n{d.get('content', '')}" for i, d in enumerate(documents))
...
def _run_pipeline_variant(question: str, conversation: list[dict], documents: list[dict], tools: list[dict]) -> PipelineResult:
    ...
    return PipelineResult(
        ...
        total_tokens=(llm_result.input_tokens or count_tokens(prompt)) + llm_result.output_tokens,
        latency_ms=llm_result.latency_ms,
        cost=0.0,  # TODO: wire up per-model pricing table once model routing (Step 6) lands
...
async def run(req: RunRequest):
    ...
    if req.mode in ("baseline", "compare"):
        ...
        if req.rubric:
            quality_baseline = judge.evaluate(req.question, baseline_result.answer, req.rubric).score

    if req.mode in ("optimized", "compare"):
        ctx = OptimizerContext(question=req.question, conversation=conversation, documents=documents, tools=tools)
        optimized_ctx = run_pipeline(ctx).context
        optimized_result = _run_pipeline_variant(
            optimized_ctx.question, optimized_ctx.conversation, optimized_ctx.documents, optimized_ctx.tools
        )
        if req.rubric:
            quality_optimized = judge.evaluate(req.question, optimized_result.answer, req.rubric).score
    ...
    if baseline_result and optimized_result:
        ...
        if baseline_result.cost > 0:
            cost_reduction = round((1 - optimized_result.cost / baseline_result.cost) * 100, 2)
